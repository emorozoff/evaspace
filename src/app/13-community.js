...
/* =====================================================================
   ...
   ник, который женщина придумывает сама при входе в комнату.

   Ник раньше считался из почты: «Участница 42». Так было безопасно, но
   безлико — номер не звучит как человек, и разговаривать с номером трудно.
   Теперь имя в комнате выбирает она, и это её единственное имя здесь:
   с её страницей, почтой и настоящим именем оно не связано ничем, а в
   соседней комнате она заводит другое.

   Из этого следует всё остальное:
   · нажать на ник и попасть на её страницу нельзя — почты в сообщении нет;
   · ник живёт только в её телефоне и в тексте её сообщений: на сервере
     нет строчки, которая связала бы ник с аккаунтом;
   · вести такую группу может только редакция: у обычной женщины не может
     быть права молчаливо узнать, кто написал;
   · модерация работает по нику — этого хватает, чтобы убрать сообщение
...
/* ---------- ник ----------
   Хранится у неё: ник по комнате и список прежних. Прежние нужны, чтобы
   после смены ника её старые сообщения остались её — узнать по ним, кто
   она, всё равно нельзя: список никуда не уходит. */
const myAnonName = gid => (S.anonNick || {})[gid] || '';
const anonPast = gid => (S.anonPast || {})[gid] || [];
const hasAnonNick = gid => !!myAnonName(gid);
const anonMine = (name, gid) => !!name &&
  (name === myAnonName(gid) || anonPast(gid).indexOf(name) >= 0);

/* Ники, уже занятые в этой комнате: смотрим по самим сообщениям — другого
   списка нет и заводить его нельзя, он стал бы перечнем участниц. */
function anonUsed(gid){
  const out = {};
  ((S.chats || {})[gid] || []).forEach(m => {
    if(m && m.anon && m.a) out[String(m.a).toLowerCase()] = true;
  });
  return out;
}
const ANON_RESERVED = ['редакция евы', 'ева', 'eva', 'eva space', 'админ', 'администратор'];
/* Что не так с ником — одной фразой её словами, а не кодом ошибки. */
function anonNickError(gid, nick){
  const n = String(nick || '').trim();
  if(n.length < 2)  return 'Ник хотя бы из двух букв';
  if(n.length > 18) return 'Слишком длинный — до восемнадцати знаков';
  if(!/^[\wа-яёА-ЯЁ][\wа-яёА-ЯЁ \-.]*$/u.test(n)) return 'Только буквы, цифры, пробел и дефис';
  if(n.indexOf('@') >= 0) return 'Без почты — это анонимная комната';
  if(ANON_RESERVED.indexOf(n.toLowerCase()) >= 0) return 'Так подписывается редакция, выбери другой';
  if(n === myAnonName(gid)) return 'Это твой нынешний ник';
  if(anonUsed(gid)[n.toLowerCase()]) return 'Такой ник в этой комнате уже занят';
  return '';
}
function setAnonNick(gid, nick){
  const n = String(nick || '').trim();
  const err = anonNickError(gid, n);
  if(err){ toast(err); return false; }
  S.anonNick = S.anonNick || {};
  S.anonPast = S.anonPast || {};
  const was = S.anonNick[gid];
  if(was && was !== n){
    S.anonPast[gid] = (S.anonPast[gid] || []).concat([was]).slice(-6);
  }
  S.anonNick[gid] = n;
  if((S.joined || []).indexOf(gid) < 0) S.joined = (S.joined || []).concat([gid]);
  S.sheet = null;
  render(); schedulePersist();
  toast(was ? 'Теперь ты здесь ' + n : 'Здесь ты ' + n);
  return true;
}
/* Три подсказки на случай «не придумывается»: слова нейтральные,
   ни возраст, ни город, ни настроение по ним не прочитать. */
const NICK_A = ['Тихая','Ясная','Тёплая','Ранняя','Поздняя','Синяя','Лунная','Снежная','Мятная','Дальняя'];
const NICK_B = ['осень','река','сова','лиса','пятница','волна','сосна','заря','гавань','искра'];
function nickIdeas(gid){
  const used = anonUsed(gid), out = [];
  for(let i = 0; out.length < 3 && i < 40; i++){
    const n = NICK_A[Math.floor(Math.random() * NICK_A.length)] + ' ' +
              NICK_B[Math.floor(Math.random() * NICK_B.length)];
    if(!used[n.toLowerCase()] && out.indexOf(n) < 0) out.push(n);
  }
  return out;
}
...
function myGRole(g){
  ...
  if(isAnon(g)) return S.role === 'admin' ? 'owner'
    : hasAnonNick(g.id) || (S.joined || []).indexOf(g.id) >= 0 ? 'member' : 'visitor';
  const me = myMail();
