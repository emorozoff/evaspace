...
/* ---------- тексты шагов ---------- */
/* Онлайн-встреча — другой разговор: нет адреса, нечего брать с собой,
   зато нужна ссылка. Поэтому каждый шаг знает формат и не обещает
   «напомнить адрес» тем, кто подключится из дома. */
const evOnline = e => !!(e && e.mode === 'онлайн');
/* строка «где»: для онлайн — платформа, для офлайн — город и адрес */
const evWhereLine = e => (evOnline(e) ? '💻 ' : '📍 ') + evPlaceText(e);
/* ссылка на встречу в письме: есть — даём, нет — честно говорим, когда будет */
function evLinkLine(e){
  return e.link ? '🔗 Ссылка на встречу: ' + e.link : '🔗 Ссылку на встречу пришлю сюда за час до начала.';
}
function evMessage(e, k){
  const st = evStateSet(e.id, {});
  const when = evWhenText(e), where = evWhereLine(e);
  const online = evOnline(e), paid = evPaid(e), unl = !!e.unlimited;
  const bring = e.bring ? '\n' + (online ? 'Как подключиться: ' : 'Что взять: ') + e.bring : '';
  const name = S.name ? S.name + ', ' : '';
  const Name = S.name ? S.name + ', ' : '';

  if(k === 'booked') return {t:
    (paid ? Name + 'билет у тебя. ' : Name + 'место за тобой. ') + '«' + e.t + '»\n\n' +
    '📅 ' + when + '\n' + where +
    (paid ? '\n🎟 ' + money(e.price) + ', оплачено' : '\n🎟 бесплатно') + '\n\n' +
    (paid
      ? (online
          ? 'Подтверждать ничего не нужно. Ссылку на встречу пришлю сюда за час до начала, а накануне напомню.'
          : 'Подтверждать ничего не нужно — просто приходи. Напомню накануне и в день встречи.')
      : (online && unl
          ? 'За три дня спрошу, будешь ли, — чтобы ведущая знала, сколько вас ждать. Ссылка придёт сюда за час до начала.'
          : online
          ? 'За три дня попрошу подтвердить, что будешь: мест немного, и на них есть очередь. Ссылка придёт сюда за час до начала.'
          : 'За три дня попрошу подтвердить, что идёшь: на бесплатных местах всегда есть очередь.')) +
    '\n\nИ один вопрос, чтобы ведущая знала, сколько вас ждать: ' +
    (online ? 'подключишься одна или позовёшь подругу?' : 'придёшь одна или с подругой?'), act:'plusone', ticket:true};

  if(k === 'invite') return st.plusOne
    ? {t:'Держи приглашение для подруги на «' + e.t + '».\n\n' +
         (online
           ? 'Она открывает ссылку, вводит код и получает свою ссылку на ту же встречу.\n'
    ...
    : {t:name + 'через неделю «' + e.t + '».\n\n' +
         (online
           ? 'Маленькое наблюдение: вдвоём доходят чаще — даже до ссылки. Если есть кого позвать — отправь подруге приглашение, ' +
             'ей придёт своя ссылка' + (paid ? ' и скидка' : '') + '. Если нет — тоже нормально, тебя ждут и одну.'
  ...
  if(k === 'confirm') return {t:
    ...
    (online ? 'Скажи, будешь? ' : 'Скажи, идёшь? ') +
    (online && unl
      ? 'Если планы поменялись — так бывает у всех, просто нажми «не смогу»: ведущая будет знать, сколько вас. ' +
        'Никто не расстроится: честный ответ дороже вежливого.'
      : 'Если планы поменялись — так бывает у всех, просто нажми «не смогу», ' +
  ...
  if(k === 'hold') return {t:
    ...
    (online
      ? 'Билет у тебя, ничего подтверждать не надо. Ссылку пришлю сюда за час до начала. ' +
        'Если хочешь, позови подругу — после встречи будет с кем обсудить.'
  ...
  if(k === 'day') return {t:
    ...
    where + bring + '\n\n' +
    (online
      /* если ведущая сама написала про ссылку в «как подключиться» — не повторяем */
      ? (/ссылк/i.test(e.bring || '') ? '' : 'Ссылку пришлю сюда за час до начала. ') +
        'Камера по желанию — можно слушать с телефона, хоть из кровати.'
  ...
  if(k === 'today') return {t:
    ...
    (online
      ? evLinkLine(e) + (e.link ? '\nЗаходи за пять минут до начала, комната уже будет открыта.' : '')
      : where) +
    '\n\nЕсли день сложился иначе — напиши сюда, это нормально.', act:'', ticket:true};

  if(k === 'after') return {t:
    name + 'ну как вчера — «' + e.t + '»?\n\n' +
    'Сначала главное: получилось ' + (online ? 'подключиться' : 'прийти') + '? Отвечай честно, это не экзамен: по таким ответам мы понимаем, ' +
...
function evPlusOne(id, yes){
  ...
  evSay(e, '', yes
    ? 'Отлично, беру два места. За неделю пришлю приглашение для подруги' + (evOnline(e) ? ' — ей придёт своя ссылка.' : '.')
    : 'Записала. Если передумаешь и захочешь позвать подругу — скажи, место найдём.', yes ? '' : 'invite');
...
function evConfirm(id, yes){
  ...
  if(yes){
    ...
    evSay(e, '', 'Записала окончательно. Ждём тебя ' + evWhenText(e) + '.\n' +
      (evOnline(e) ? 'Накануне напомню, а ссылку на встречу пришлю сюда за час до начала.'
    ...
    /* подтверждение спрашиваем только на бесплатных — про возврат денег
       здесь говорить нечего; на встрече без лимита и «место уйдёт» ни к чему */
    evSay(e, '', (e.unlimited
        ? 'Спасибо, что сказала заранее — ведущая будет знать, сколько вас. Запись отменена.'
        : 'Спасибо, что сказала заранее — место уйдёт другой женщине. Запись отменена.') +
      (evPaid(e) ? '\nДеньги за билет вернутся тем же способом в течение трёх дней.' : '') +
      '\nЗахочешь вернуться — запишись снова, если место ещё будет.', '');
  }
  render(); schedulePersist();
}
...
function evCame(id, yes){
  const e = evById(id); if(!e) return;
  evAnswer(id, yes ? 'Да, была' : (evOnline(e) ? 'Не получилось подключиться' : 'Не получилось прийти'));
...
function shTicket(){
  ...
  ${invite
    ? `<p class="small muted" style="margin:14px 0 12px">Перешли этот код подруге. Она вводит его
        при записи на «${esc(e.t)}» и ${online ? 'получает свою ссылку на ту же встречу' : 'садится рядом с тобой'}${paid ? ', со скидкой по приглашению' : ''}.</p>
    ...
    : `<p class="small muted" style="margin:14px 0 12px">${online
        ? (paid ? 'Подтверждать участие не нужно. ' : 'За три дня спрошу, будешь ли. ') +
          (e.link ? 'Ссылка на встречу ниже — она же придёт в письме за час до начала.' : 'Ссылка на встречу придёт в письме от Eva Events за час до начала.')
        : paid
       ...
       ${online && e.link ? `<a class="btn" style="display:block;text-align:center;text-decoration:none;margin-bottom:9px" href="${safeUrl(e.link)}" target="_blank" rel="noopener">Открыть ссылку на встречу</a>` : ''}
