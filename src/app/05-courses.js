...
/* Отметка «пройден» — её личный прогресс, а не свойство урока: уроки
   общие для всех и приезжают с сервера, при каждой синхронизации они
   заменяются. Раньше флаг лежал прямо в уроке и терялся вместе с ним. */
const lessonDone = id => !!((S.courseDone || {})[id]);
function courseProgress(cid){
  const ls = lessonsOf(cid);
  const done = ls.filter(l => lessonDone(l.id)).length;
  const next = ls.findIndex(l => !lessonDone(l.id));
  return {ls, done, total:ls.length, pct: ls.length ? Math.round(done / ls.length * 100) : 0,
          next: next < 0 ? ls.length - 1 : next, all: !!ls.length && done === ls.length};
}
const courseOwned = cid => (S.courses || []).indexOf(cid) >= 0;
/* модуль, в котором лежит урок */
function moduleOfLesson(cid, n){
  const mods = (typeof modulesOf === 'function') ? modulesOf(cid) : [];
  return mods.find(m => (m.units || []).indexOf(n) >= 0) || null;
}
  ...
  return `<div class="view pad">
    ...
    <div class="chead">
      ...
      <div class="ctext">
        ...
        <div style="font-size:12.5px;color:rgba(255,255,255,.78);margin-top:6px">${esc(c.e)} · ${plural(ls.length,'урок','урока','уроков')}</div>
      </div>
    </div>

    ${own ? `<div class="card cont">
      <div class="spread"><div style="flex:1"><b style="font-size:14.5px">${pr.all ? 'Курс пройден' : 'Ты на курсе'}</b>
    ...
    <div class="card">
      <b style="font-size:14.5px">О курсе</b>
      <p style="font-size:14px;line-height:1.55;margin:7px 0 0;white-space:pre-line">${esc(about)}</p>
      ${info.promo && info.promo !== about ? `<p class="small muted" style="margin:8px 0 0">${esc(info.promo)}</p>` : ''}
      <div class="chips wrap" style="margin-top:10px;padding-bottom:0">${(COURSE_TAGS[c.id]||[]).map(t => `<span class="chip pale">${esc(t)}</span>`).join('')}</div>
    </div>

    ${info.who.length ? `<div class="sec-h"><h2 class="serif">Для кого этот курс</h2></div>
    ...
    ${info.gives.length ? `<div class="sellwrap">
    ...
    <div class="sec-h"><h2 class="serif">Программа</h2>
      <span class="small muted">${plural(mods.length,'модуль','модуля','модулей')} · ${plural(ls.length,'урок','урока','уроков')}</span></div>
    ${mods.map(m => `<div class="modbox">
      <div class="mh"><span>Модуль ${esc(m.n)}. ${esc(m.t)}</span>
        <span class="lockmini">${plural(m.units.length,'урок','урока','уроков')}</span></div>
      <div style="padding:8px 8px 1px">
        ${m.units.map(un => { const l = ls.find(x => x.n === un); return l ? unitRow(c, l) : ''; }).join('')}
      </div>
    </div>`).join('')}
    ...
    ${own ? '' : `<div class="card" style="margin-top:14px">
...
function doneUnit(cid, i){
  const ls = lessonsOf(cid), l = ls[i];
  if(!l || lessonDone(l.id)) return;
  S.courseDone = S.courseDone || {};
  S.courseDone[l.id] = Date.now();
  S.points += 20;
  const pr = courseProgress(cid);
...
/* типы мероприятий: один список для формы, правки и фильтров */
const EVENT_KINDS = ['Женский круг','Встреча','Практика','Лекция','Концерт',
...
const EVENTS = [
  ...
  {id:'ev1', t:'Женский круг «Возвращение к себе»', kind:'Женский круг', d:'2026-09-11', tm:'19:00',
   ...
   full:'Три часа, за которые тебя никто не перебьёт.\n\n' +
        'Круг на двенадцать женщин. Каждая говорит столько, сколько хочет: перебивать, ' +
        'советовать и «а вот у меня было» — нельзя. Это единственное жёсткое правило, ' +
        'и именно оно делает круг кругом.\n\n' +
        '## Что будет\n' +
        '- чай и знакомство без формальностей\n' +
        '- первый круг: с чем ты пришла\n' +
        '- телесная практика, чтобы разговор не остался в голове\n' +
        '- письмо себе, которое заберёшь домой\n\n' +
        '## Важно\n' +
        'Круг закрытый: *то, что сказано внутри, остаётся внутри*. ' +
        'Можно прийти и промолчать всю встречу — это тоже участие.',
  ...
  {id:'ev13', t:'Женский завтрак на Патриках', kind:'Женский завтрак', d:'2026-09-13', tm:'10:30',
   mode:'офлайн', city:'Москва', place:'кофейня «Сад», Малая Бронная 22', price:1200, by:'Тая Мирная',
   seats:10, left:3, status:'live',
   about:'Завтрак вдесятером и один вопрос на всех. Два часа, и ты знаешь девять новых женщин.',
   full:'Формат простой: садимся завтракать, и я задаю один вопрос. Каждая отвечает столько, ' +
        'сколько хочет.\n\n' +
        'Вопрос сентября: *что ты перестала терпеть в этом году?*\n\n' +
        '## Почему это работает\n' +
        '- десять человек — все успевают сказать\n' +
        '- два часа — не надо отпрашиваться на весь день\n' +
        '- один вопрос — разговор не рассыпается на светскую болтовню\n\n' +
        'Завтрак оплачивается отдельно, взнос — за место и ведение.',
   program:['10:30 — собираемся, заказываем','10:45 — знакомство по кругу, одна фраза о себе',
            '11:00 — вопрос месяца и ответы','12:10 — свободный разговор','12:30 — расходимся'],
   who:'Тем, кто устал от переписок и хочет живых разговоров.',
   bring:'Ничего. Приходи как есть, стол уже забронирован на твоё имя.'},

  {id:'ev14', t:'Девичник Ева: вечер без телефонов', kind:'Девичник Ева', d:'2026-09-25', tm:'19:00',
   mode:'офлайн', city:'Москва', place:'лофт «Круг», Большая Новодмитровская 36', price:3200,
   by:'Ольга Светлова', seats:24, left:11, status:'live',
   about:'Наш фирменный вечер: игры на честность, танцы и фотограф. Телефоны сдаём на входе.',
   full:'Девичник Ева — это то, чего не хватает взрослым женщинам: вечер, где можно ' +
        'громко смеяться и не следить за лицом.\n\n' +
        '## Как проходит вечер\n' +
        '- на входе отдаёшь телефон и получаешь бокал\n' +
        '- игра на честность в маленьких кругах по шесть человек\n' +
        '- танцевальный час, свет приглушён, зеркал нет\n' +
        '- фотограф снимает весь вечер, снимки придут через три дня\n\n' +
        '## Что важно знать\n' +
        'Приходить можно одной — за столом ты точно не останешься одна, мы рассаживаем сами. ' +
        '*Можно взять подругу*, для неё будет скидка по приглашению из приложения.',
   program:['19:00 — встреча, телефоны в коробку','19:30 — игра на честность в кругах',
            '20:45 — перерыв и фотосессия','21:15 — танцы','23:00 — забираем телефоны и такси'],
   who:'Всем, кому больше 25 и кто давно не был на настоящем девичнике.',
   bring:'Обувь, в которой ты танцуешь, и готовность к честным вопросам.'},

  {id:'ev11', t:'Ретрит выходного дня «Тишина»', kind:'Ретрит', d:'2026-09-27', tm:'10:00',
   ...
   full:'Два дня без телефона, без расписания чужих дел и без вопроса «мам, где мои носки».\n\n' +
        '## Как это устроено\n' +
        'Телефоны утром складываем в коробку и забираем на второй день после обеда. ' +
        'Это добровольно — но именно из-за этого ретрит и работает.\n\n' +
        '## Что входит\n' +
        '- проживание в двухместных комнатах\n' +
        '- питание, вегетарианское и очень вкусное\n' +
        '- трансфер от метро Тушинская и обратно\n' +
        '- баня в субботу вечером\n\n' +
        'Мест всего шестнадцать — *больше не берём*, иначе это уже не тишина.',
  ...
  const dayHtml = d => {
    ...
    return `<div class="pday${d.at === today.getTime() ? ' now' : ''}${d.past ? ' gone' : ''}">
      ...
      <div class="pitems">${d.items.map(e => {
        ...
        const on = e.mode === 'онлайн';
        return `
        <button class="pline" onclick="openSheet({k:'event',id:'${attJs(e.id)}'})">
          <span class="ptxt">
            <span class="phead"><b>${esc(e.t)}</b>${
              (typeof evBadge === 'function' ? evBadge(e.id) : '') ||
              (mark ? `<span class="psoon">${mark}</span>` : '')}</span>
            <span class="pmeta">
              <i class="pdot ${on ? 'on' : 'off'}"></i>${esc(e.tm)} · ${
                on ? 'онлайн' : 'офлайн' + (e.city ? ', ' + esc(e.city) : '')}</span>
...
const CHAT_SEED = {
  gr10:[['Редакция Евы','#2E2145',"Здесь без имён. Можно писать о том, о чём не говорят вслух: никто не увидит, кто ты. Мы читаем комнату каждый день.",'09:00','','1'],
...
/* В анонимной комнате первое, что нужно сделать, — назваться: без ника
   она не может написать. Поэтому окно с ником открывается само, один раз
   за заход. Закрыла — читает дальше, кнопка и карточка остаются. */
const NICK_ASKED = {};
function openChat(id){
  initChats();
  S.chat.open = id; (S.chat.unread||{})[id] = 0;
  const g = GROUPS.find(x => x.id === id);
  if(g && isAnon(g) && S.role !== 'admin' && !hasAnonNick(id) && !NICK_ASKED[id]){
    NICK_ASKED[id] = true;
    S.sheet = {k:'anonNick', id};
  }
  render(); scrollChat();
}
...
function sendMsg(gid){
  ...
  if(anon && anonMuted(g, myAnonName(gid)))
    return toast('Здесь ты больше не пишешь. Напиши в поддержку, если это ошибка');
  const now = new Date();
  const tm = String(now.getHours()).padStart(2,'0')+':'+String(now.getMinutes()).padStart(2,'0');
...
/* Первое, что она видит в «Сообщениях». Раньше здесь лежали два
   выдуманных письма — от «Ирины» и от эксперта, как будто ей уже написали
   живые люди. В демонстрации это смотрелось живо, на боевой платформе —
   это обман: она отвечает, а там никого. Оставляем одно настоящее письмо
   от Евы: оно объясняет, кто сюда пишет, и ждать ответа от него не нужно. */
function seedInbox(){
  S.inbox = S.inbox || [];
  if(S.inbox.some(x => x.id === 'eva_week')) return;
  S.inbox.push({id:'eva_week', from:'Ева', c:'#A8375C', kind:'программа', chan:'eva',
    ago:'только что', unread:true, sys:true, msgs:[{me:false, tm:'08:00', t:
      'Это твои сообщения. Сюда придут итоги недели от меня, письма про встречи, ' +
      'ответы по заказам и на заявки — у каждого свой отправитель, их видно по значку. ' +
      'Здесь же переписка с участницами и ответы экспертов. ' +
      'На любое письмо можно ответить прямо в нём — прочитаем.'}]});
}
    ...
    ${S.inbox.some(x => x.kind === 'мероприятие') && S.evFast !== false ? `
      <div class="evfast"><span>⏱</span>
        <div><b>Ускоренный показ включён</b>
          Письма Eva Events приходят раз в десять секунд, чтобы можно было посмотреть всю цепочку.
          У каждого письма подписано, за сколько до мероприятия оно придёт на самом деле.
...
function sendDM(id){
  ...
  if(t.kind === 'маркет'){
    ...
    syncPush(['questions']);
    render(); schedulePersist();
    return toast('Вопрос отправлен. Ответ придёт в сообщения');
  }
