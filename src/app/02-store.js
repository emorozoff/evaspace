...
function dayStory(){
  ...
  const out = [];
  out.push(`${m.n}, освещённость ${m.pct}% — ${m.age} день лунного цикла. ${m.s}`);
  if(z){
    const key = EL_MOON[m.n] ? m.n : m.age < 14 ? 'Растущая луна' : 'Убывающая луна';
    const el = (EL_MOON[key] || {})[z.el];
    out.push(`Ты ${z.n}, стихия ${z.el}.` + (el ? ` В такую луну тебе ${el}.` : '') +
             ` ${zodTip(z)}`);
  }
  if(cy) out.push(`Цикл: ${cy.phase.n.toLowerCase()} фаза, день ${cy.day} из ${cy.len}. ` +
                  `${cy.phase.s} До следующего — ${plural(cy.next, 'день', 'дня', 'дней')}.`);
  return out;
}
