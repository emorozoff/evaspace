...
function checkout(){
  ...
  platformSay(`${S.name ? S.name + ', заказ' : 'Заказ'} у нас: ${items.map(i => i.t + (i.n > 1 ? ' × ' + i.n : '')).join(', ')}. ` +
    `Сумма ${money(total)}${used ? ', из них бонусами ' + used : ''}. Номер ${oid} — по нему найдём за секунду.\n\n` +
    `Менеджер напишет сюда, как оплатить и когда привезём, — обычно в тот же день. Передумала или хочешь ` +
    `что-то добавить — просто ответь на это письмо. Кэшбэк ${cb} бонусов уже на счету.`,
    '', 'market');
...
function buyCourse(id){
  ...
  platformSay(`${S.name ? S.name + ', заявка' : 'Заявка'} на «${c.t}» у нас. Оплата картой ещё подключается, поэтому ` +
    `доступ откроем руками и напишем сюда — обычно в течение дня. А первые уроки открыты уже сейчас: ` +
...
function sendConsult(eid){
  ...
  platformSay(`${S.name ? S.name + ', передала' : 'Передала'} ${e ? e.n : 'эксперту'} — она увидит заявку сегодня. ` +
    `Свяжемся по «${contact}» и напишем сюда, когда договоримся о времени. ` +
    `Если удобно только в определённые часы — напиши их ответом на это письмо.`,
    '', 'experts', t && t.id);
  S.sheet = null; render();
  toast('Заявка отправлена. Подтверждение — в сообщениях');
}
...
function sendExpertQuestion(eid){
  ...
  platformSay(`Передала ${e ? e.n : 'эксперту'} лично: «${text.length > 90 ? text.slice(0, 88) + '…' : text}». ` +
    `Она отвечает обычно в течение суток — ответ придёт сюда, сидеть у экрана не нужно.`, '', 'experts', tk && tk.id);
  S.sheet = null; render();
...
function askGood(gid){
  ...
  th.qid = id;
  platformSay(`Вопрос про «${g ? g.t : 'товар'}» ушёл менеджеру маркета — ответит сюда, обычно в течение дня. ` +
    'Если это про уже сделанный заказ, напиши его номер: так найдём быстрее.', '', 'market');
...
function sendSupport(){
  ...
  platformSay(`${S.name ? S.name + ', получили' : 'Получили'} обращение «${S.supTopic || 'Другое'}». Читает живой человек, ` +
    `отвечаем обычно в течение дня — ответ придёт сюда. Если срочно, напиши «срочно» ответом: поднимем выше в очереди.`,
    '', 'space', tk && tk.id);
...
function sendExpertApply(){
  ...
  INBOX.unshift(tk);
  S.expApply = null;
  platformSay('Заявка на роль эксперта у нас — спасибо, что решились. Прочитаем внимательно, не по диагонали, ' +
    'и ответим сюда в течение недели. Дальше один шаг — встреча в Zoom с куратором: полчаса разговора о том, ' +
    'что вы делаете и для кого. Из таких заявок и растёт Ева.', '', 'experts', tk.id);
...
function sendPartnerApply(){
  ...
  S.partnerApply = null;
  platformSay(`Заявка партнёра «${d.brand.trim()}» получена. Посмотрим товар глазами покупательницы и ответим сюда ` +
    'в течение трёх дней. Маркет мы собираем из вещей, которые купили бы сами, — поэтому и смотрим внимательно, ' +
    'а не для галочки.', '', 'market', t.id);
  S.sheet = null; render();
  toast('Заявка отправлена. Подтверждение — в сообщениях');
}
...
function sendEventApply(){
  ...
  platformSay(own
    ? `Мероприятие «${d.t.trim()}» получено. Проверим по правилам площадки — безопасность, формат, кто ведёт — и, если всё сходится, опубликуем. Напишем сюда в течение трёх дней; если чего-то не хватит, спросим здесь же.`
    : `Спасибо за наводку на «${d.t.trim()}» — такие рекомендации для нас дороже рекламы. Свяжемся с организаторами и, если подойдёт, добавим в раздел. Напишем сюда, чем кончилось.`,
    '', 'events', t.id);
  S.sheet = null; render();
