...
function payPlan(){
  ...
  S.subAsked = true;
  platformSay(`${S.name ? S.name + ', заявка' : 'Заявка'} на подписку (${plan}) получена. Оплата картой ещё подключается, ` +
    'поэтому доступ откроем руками и напишем сюда — обычно в течение дня. Сразу после этого откроются вся библиотека, ' +
    'курсы и клуб — ничего дополнительно нажимать не придётся.', '', 'space', tk.id);
