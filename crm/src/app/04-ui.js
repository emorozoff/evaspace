...
function helpBox(key, html) {
  const hidden = Local.get('eva-crm-help', {})[key];
...
function wireHelp(root) {
  on(root, 'click', '[data-help-hide]', (e, el) => {
    const m = Local.get('eva-crm-help', {}); m[el.dataset.helpHide] = 1; Local.set('eva-crm-help', m);
    const box = $(`[data-help="${el.dataset.helpHide}"]`, root); if (box) box.hidden = true;
  });
  on(root, 'click', '[data-help-show]', (e, el) => {
    const m = Local.get('eva-crm-help', {}); delete m[el.dataset.helpShow]; Local.set('eva-crm-help', m);
...
/* график в половинной карточке: на узком экране карточки встают в столбик */
const halfWidth = () => { const w = ($('#page') || {}).clientWidth || 800; return w < 1000 ? chartWidth() : clamp(Math.round(w / 2 - 60), 300, 640); };
...
function barChart({labels, values, plan = null, height = 200, color = 'var(--rose)', yFmt = fmt, highlight = -1, half = false}) {
  const W = half ? halfWidth() : chartWidth(), H = height, L = 40, R = 12, T = 12, B = 26;
  const n = labels.length;
...
function wireCharts() {
  ...
  document.addEventListener('pointermove', e => {
    ...
    if (box.dataset.chart === 'line') {
      ...
      if (box.dataset.chart === 'stack') {
        tipAt(box, `<div class="tip-h">${esc(label)}</div><div class="tip-row"><b>${esc(val)}</b><span>всего</span></div>${(plan || '').split(' · ').filter(Boolean).map(x => `<div class="tip-row"><span>${esc(x)}</span></div>`).join('')}`, e.clientX - rect.left, e.clientY - rect.top);
        return;
      }
...
/* ── каналы связи ── */
const CH_ICON = {
  tg: '<path d="M21 4 3 11l6 2.2L18 7l-7 7.4V20l3.3-3.6L18.5 19z"/>',
  wa: '<path d="M4 20l1.3-3.9A8 8 0 1 1 8 19z"/><path d="M9 8.5c0 3 2.5 6 5.5 6.5l1.3-1.3-2-1-1 .8a4 4 0 0 1-2.3-2.3l.8-1-1-2z"/>',
  max: '<path d="M4 18V6l8 7 8-7v12"/>',
  email: ICONS.mail,
...
function chBadge(ch, withName = false) {
  const c = CHANNELS[ch] || {name: ch, color: 'var(--ink-3)'};
  return `<span class="ch ch-${esc(ch)}" style="--c:${c.color}" title="${esc(c.name)}"><svg viewBox="0 0 24 24" aria-hidden="true">${CH_ICON[ch] || ''}</svg>${withName ? `<b>${esc(c.short || c.name)}</b>` : ''}</span>`;
}

/* ── этапы, теги, сегменты ── */
function stagePill(funnelId, stageId) {
  const f = Funnels.get(funnelId), s = Funnels.stage(funnelId, stageId);
  if (!f || !s) return '<span class="pill">без этапа</span>';
  if (stageId === 'lost') return `<span class="pill bad">${esc(s.name)}</span>`;
  const i = f.stages.findIndex(x => x.id === stageId);
  const tone = s.won ? 'good' : i >= f.stages.length - 3 ? 'gold' : i >= 2 ? 'violet' : '';
  return `<span class="pill ${tone}"><i class="st-n">${i + 1}</i>${esc(s.name)}</span>`;
}
function tagChip(t, removable = false) {
  const tg = Tags.get(t);
  const color = tg ? tg.color : '#7B7987';
  return `<span class="tagc" style="--c:${color}">${esc(tg ? tg.name : t)}${removable ? `<button type="button" data-untag="${esc(t)}" aria-label="Убрать тег">×</button>` : ''}</span>`;
}
const tagsHtml = (list, max = 3) => {
  const a = list || [];
  return a.slice(0, max).map(t => tagChip(t)).join('') + (a.length > max ? `<span class="tagc more">+${a.length - max}</span>` : '');
};
function segPill(seg) {
  if (!seg) return '<span class="muted">—</span>';
  return `<span class="pill line"><i class="dot" style="background:${esc(seg.color)}"></i>${esc(seg.name)}</span>`;
}
...
function stackChart({labels, series, height = 220, yFmt = rubK, half = false}) {
  const W = half ? halfWidth() : chartWidth(), H = height, L = 56, R = 12, T = 12, B = 26;
  const n = labels.length;
  const totals = labels.map((_, i) => sum(series, s => Math.max(0, s.values[i] || 0)));
  const sc = niceScale(Math.max(1, ...totals), false);
  const band = (W - L - R) / Math.max(1, n), bw = Math.min(34, Math.max(6, band - 10));
  const y = v => T + (1 - v / sc.max) * (H - T - B);
  let g = '';
  for (let v = 0; v <= sc.max + 1e-9; v += sc.step) g += `<line class="grid" x1="${L}" x2="${W - R}" y1="${y(v)}" y2="${y(v)}"/><text x="${L - 8}" y="${y(v) + 4}" text-anchor="end">${esc(yFmt(v))}</text>`;
  let marks = '';
  labels.forEach((lb, i) => {
    const cx = L + band * i + band / 2;
    g += `<text x="${cx}" y="${H - 6}" text-anchor="middle">${esc(lb)}</text>`;
    let acc = 0;
    const parts = [];
    series.forEach(s => {
      ...
      marks += `<rect x="${cx - bw / 2}" y="${y1}" width="${bw}" height="${Math.max(0, y0 - y1 - (acc ? 1 : 0))}" fill="${s.color}" rx="2"/>`;
      parts.push(`${s.name}: ${yFmt(v)}`);
      acc += v;
    });
    marks += `<g class="bar-hit" data-tip="${esc(`${lb}|${yFmt(totals[i])}|${parts.join(' · ')}`)}" tabindex="0"><rect x="${cx - band / 2}" y="${T}" width="${band}" height="${y(0) - T}" fill="transparent"/></g>`;
  });
  return `<div class="chart-box" data-chart="stack"><svg class="chart" viewBox="0 0 ${W} ${H}" role="img">${g}<line class="axis" x1="${L}" x2="${W - R}" y1="${y(0)}" y2="${y(0)}"/>${marks}</svg><div class="chart-tip" hidden></div></div>
    <div class="legend">${series.map(s => `<span><i style="background:${s.color}"></i>${esc(s.name)}</span>`).join('')}</div>`;
}

/* запись звонка: настоящий файл — <audio>, в демо — полоса-заглушка */
function recPlayer(ev) {
  if (ev.rec && /^(https?:|blob:|data:)/.test(ev.rec)) return `<audio class="rec-audio" controls preload="none" src="${esc(ev.rec)}"></audio>`;
  const bars = Array.from({length: 38}, (_, i) => 20 + ((hashStr(ev.id + i) % 70))).map(h => `<i style="height:${h}%"></i>`).join('');
  return `<div class="rec-demo" title="Демо-запись: настоящие записи придут из телефонии"><span class="rec-play">${icon('play')}</span><span class="rec-wave">${bars}</span><span class="num">${dur(ev.dur)}</span></div>`;
}

/* поле «только для чтения» в карточке */
